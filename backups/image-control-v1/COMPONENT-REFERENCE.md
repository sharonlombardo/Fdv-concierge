# Image Control v1 - Component Reference

**Backup Date:** January 12, 2026

This file contains the complete code for the Image Control page and image slots configuration. This is your safekeeping backup that you can download.

## Files Included:
1. `client/src/pages/image-control.tsx` - The Image Control page component
2. `shared/image-slots.ts` - All image slot definitions (150+ slots)

---

## File 1: image-control.tsx
...
## File 2: image-slots.ts (Key Structure)

The image-slots.ts file contains 150+ image slots organized by section. Here is the structure:

### Slot Categories:
- **The Current: Opening** - Magazine cover and Today's Edit tiles
- **The Current: Morocco** - Morocco story images (hero, style, texture, tiles, objects)
- **The Current: Hydra** - Hydra story images
- **The Current: Slow Travel** - Slow travel story images
- **The Current: Retreat** - Retreat story images
- **The Current: New York** - New York story images
- **Trip Transition** - Transition frame images
- **Today's Edit** - Mood and looks grid images
- **Landing Hero** - Landing page hero images
- **Morocco Editorial: Day 1-8** - Each day has:
  - `d*-hero` - Day header image (large image at top of section)
  - `d*-1`, `d*-2`, etc. - Numbered event images
  - `d*-*-wardrobe` - Wardrobe images for events
  - `d*-*-extra-0` through `d*-*-extra-3` - Accessory images

### Slot Key Format:
```
d1-hero          → Day 1 header image
d1-1             → Day 1, Event 1 (e.g., "1. Marrakech Menara Airport")
d1-1-wardrobe    → Day 1, Event 1 wardrobe
d1-2             → Day 1, Event 2
d1-2-extra-0     → Day 1, Event 2 accessory slot 1
```
...
```typescript
{
  ...
  defaultUrl: "https://images.unsplash.com/photo-...",
  aspectRatio: "16:9",
...
1. Copy the `image-control.tsx` content to `client/src/pages/image-control.tsx`
2. The image-slots.ts file is too large to include in full here - it lives at `shared/image-slots.ts`
3. Make sure the route is registered in `client/src/App.tsx`

---

## Architecture Notes

- **Image slots** are defined in `shared/image-slots.ts` and served via `/api/image-slots`
- **Custom images** are stored as base64 in the database and override defaults
- **Categories** are derived from section names using `getCategoryFromSection()`
- **Accordion UI** allows collapsing sections to reduce scrolling
- **Sticky sidebar** on desktop provides quick navigation to categories
